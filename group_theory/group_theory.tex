...
We can also define an action that involves a function $S\times G \to S$ with $s,g \mapsto (s)g$ satisfying the axioms as described above. This is called a right action as opposed to the left action that was described prior. 
...
A \defn{homomorphism} is a map $f:G\to H$ that preserves multiplication:
$$
f(ab) = f(a)f(b).
$$
...
$$
0\to\Z/2\Z \to \Z/2\Z \times \Z/2\Z\to \Z/2\Z \to 0
$$
is exact since the sequence 
$$
0\to A\to A\times B \to B\to 0
$$
...
Suppose $G$ has subgroups $A,B$. Suppose $A$ and $B$ commute and every element in $G$ is of the form $ab$ with $a\in A$ and $b\in B$ uniquely. Then $G\isomto A\times B$, since there is a homomorphism $A\times B\to G$ that takes $(a,b)$ to $ab$, and this homomorphism is bijective. 
...
$$
1\to H\isomto \Z/4\Z \to G\to \Z/2\Z \to 1.
$$
...
\section{Quaternions}
Recall that $\Qu = \{ \pm1,\pm i,\pm  j,\pm k\}$ with the relations $ij = k$, $jk=i$, $ki=j$, and $i^2 = j^2 = k^2 = ijk =-1$. We have the representation $\rho : \Qu \injto \SU(2)$ where
\begin{align*}
...
$$
1\to (-1,-1)\to S^3 \times S^3 \to \SO_4(\R) \to 1
$$
...
Look at $G/\ang g$ where $g^p = 1$, $g\neq 1$, and $g$ is in the centre. Pick a Sylow $p$-subgroup $S$ of $G/\ang{g} $, so $\abs{S} = p^{n-1}$. We have the map $G\to G/\ang{g}$, and we consider the inverse image $S'$ of $S$ under this map. The inverse image $S'$ has order $p^n$, so is a Sylow $p$-subgroup of $G$.
\end{proof}
...
$$
1 \to \Zmod 2 \to \widehat{\Ag_4} \to \Ag_4 \to 1 
$$
...
$$
Indeed, we have a homomorphism $\Sg_n \to \{\pm 1\}$ where the sign depends on the sign of the above polynomial. The group $\Ag_n$ is the kernel of this homomorphism, so we have the exact sequence
$$
1 \to \Ag_n \to \Sg_n \to \{\pm 1\} \to 1.
$$
...
These groups are analogues of the Heisenberg group. Suppose $f:\R\to\C$. We can transform $f$ by $T_\alpha : f(x)\mapsto f(x+\alpha)$ and $T_\beta : f(x) \mapsto e^{2\pi i \beta x}f(x)$ (corresponding to momentum and position operators). We have $T_\alpha T_\beta \equiv T_\beta T_\alpha e^{2\pi i \alpha \beta}$. We get a three dimensional group of transformations where $f(x) \mapsto e^{2\pi i (\gamma + \beta x)} f(x+\alpha) $. The circle group $S^1$ is a subgroup, so we have the extension
$$
\{1 \} \to S^1 \to G \to \R\times \R \to \{1\}.
$$
Now suppose $f: \F_p \to \C$ with $T_\alpha : f(x) \to (x+\alpha)f(x)$ and $T_\beta : f(x) \mapsto e^{2\pi i \beta x/p} f(x)$, where $\alpha,\beta\in \F_p$. Also, $e^{2\pi i\beta x/p}$ is a well defined $p$th root of unity. These commute with each other up to multiplication by $e^{2\pi i \gamma/p}$, $\gamma\in \F_p$. We get a group $G$ of order $p^3$ where
$$
\{1\} \to \F_p \to G \to \F_p\times \F_p\to\{1\}.
$$
...
$$
\{1\} \to S^1 \to G \to \R^n \times \R^n \to \{1\},
$$
hence the extension
$$
\{1\} \to \F_p \to H \to \F_p^n \times \F_p^n \to \{1\}.
$$
...
$$
which has a quadratic form $q$, and a map $(\Zmod 2)^{2n} \to \Zmod 2 = Z$, where $q(a) = a^2$. Also, $q(ab) = q(a)q(b)\langle a,b\rangle = q(a)q(b)aba\inv b\inv$. Quadratic forms $q : \F_2^n \to \F_2$ can be distinguished by the Arf invariant, given by the most common value of $q$. (It is sometimes called the ``democratic invariant.'')
...
$$
\tau : H\textsuperscript{ab} \leftarrow G\textsuperscript{ab},
$$
...
If $G$ is abelian, if $H\in \ZZ(G)$, or if $H$ has order $2$, then $\tau(g) = g^{[G:H]}$ for $g\in H$. If $H$ has order $2$, then $\tau$ acts on $H$ as  $g\mapsto g^{[G:H]}$. If $[G:H]$ is odd, then $\tau : G\to H$ is onto. That is, if $\abs{G}= 2n$ with $n$ odd, there is a surjective homomorphism $\tau : G\to H$ where $H$ is a Sylow subgroup. So $G$ is a semidirect product $K\ltimes H$ where $K$ is index $2$ and $H\isomto \Zmod 2$, and, in particular, $G$ is not simple. No group of order $2\pmod 4$ is simple! 

Suppose $p$ is the smallest prime dividing $\abs{G}$. If $p^2$ does not divide $\abs G$, then the Sylow $p$-subgroup is $\Zmod p$, and no $2$ distinct elements can be conjugate (if there was such a pair, then there would have to be an inner automorphism of $G$ acting on $\Zmod p$ nontrivially, but no nontrivial elements of $G$ have order less than $p$). So $\tau : G\to \Zmod p$ acts as $g\mapsto g^{\abs G /p}$ for $g\in \Zmod p$, and this map is onto. So $G$ has a normal subgroup of index $p$ (a normal $p$ conjugate), so $G$ is not simple. In fact, this holds even if $p^2$ divides $\abs G$, provided the Sylow $p$-subgroup is cyclic.
...
How many ways can we write the commutator? There are about $m'(m'-1)/2$ ways of choosing $i,j$. The dimension of all bilinear maps from $(\Zmod p)^{m'} \tensor (\Zmod p)^{m'} \to (\Zmod p)^m$ is about $mm'^2 / 2$. So the vector space of all of these maps has size about $p^{mm'^2/2}$. Now, we have $m+m'=n$, where $\abs{G} = p^n$, and we want to maximize $p^{mm'^2/2}$. We get $m' \approx (2/3)n$, $m \approx (1/3)n$. So we seem to be getting about $p^{(2/27) n^3}$ groups, but there are actually less.
...
$$
This gives a surjective homomorphism $\GL_2(\F_3) \to \Sg_4$ with kernel $\{\pm I\}$:
$$
\{1 \} \to \{\pm I\} \to \GL_2(\F_3) \to \Sg_4 \to \{1\}.
$$
...
The group $\SL_2(\F_4)$ acts on $\F_4 \cup \{ \infty\}$ (the projective line), so we get a homomorphism $\SL_2(\F_4)\to \Sg_5$, and one can check that the image of the homomorphism is $\Ag_5$. The group of rotations of an icosahedron acts on $5$ objects. The dodecahedron has the same rotation group as the icosahedron since the two are dual. There are five ways to embed a cube in a dodecahedron, so each rotation of a dodecahedron gives a permutation of these five cubes. Also, refer to figure 3. The rotations of an icosahedron (or dodecahedron) give permutations of the five tetrahedra. Once again, the image of the homomorphism from the group of rotations to $\Sg_5$ is $\Ag_5$.
\begin{figure}
\centering
...
$$
\{1\} \to \ZZ(G) \to G\to \Aut(G) \to \Out(G) \to \{1\}.
$$
...
For most groups, the automorphisms are usually easy to find. Suppose $G = \PSL_n(\F_q)$. Then $\Aut(G)$ consists of inner automorphisms, conjugations by elements of $\GL_n(\F_q)$,  field automorphisms $\F_q\to \F_q$, and $g\mapsto \tp{(g\inv)}$. Indeed, $\Aut(G)$ is generated by these obvious automorphisms, and this is usually the case.
...
Let's consider outer automorphisms of $\Sg_6$. Observe that $\Sg_5$ has a subgroup of index $6$ of order $20$, which is a Frobenius group of order $20$. This gives a nontrivial homomorphism $\Sg_5\to \Sg_6$ ($\Sg_5$ acts transitively on a set of $6$ elements), so $\Sg_5\subset \Sg_6$ not fixing a point. In fact, $\Sg_6$ has $6+6$ subgroups of $\Sg_5$ (usually $\Sg_n$ has $n$ subgroups $\Sg_{n-1}$). The group $\Sg_6$ has a transitive $\Sg_5$ subgroup of index $6$. This gives a homomorphism $\Sg_6\to\Sg_6$ acting on the $6$ cosets of this subgroup. This is not an inner automorphism, so $\Sg_6$ has an outer automorphism. 
...
\begin{tikzpicture}[x=1cm,y=1cm]
\def\Factor{1}
\node[minimum size=0.2cm,
shape=circle,
fill=green!50,
font=\footnotesize] 
(a 1 1) at (\Factor*1,-\Factor*1) {};
\node[minimum size=0.2cm,
shape=circle,
fill=green!50,
font=\footnotesize] 
(a 1 2) at (\Factor*1,-\Factor*2) {};
\node[minimum size=0.2cm,
shape=circle,
fill=green!50,
font=\footnotesize] 
(a 1 3) at (\Factor*1,-\Factor*3) {};
\node[minimum size=0.2cm,
shape=circle,
fill=green!50,
font=\footnotesize] 
(a 1 4) at (\Factor*1,-\Factor*4) {};
\node[minimum size=0.2cm,
shape=circle,
fill=green!50,
font=\footnotesize] 
(a 1 5) at (\Factor*1,-\Factor*5) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 2 1) at (\Factor*2,-\Factor*1) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 2 2) at (\Factor*2,-\Factor*2) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 2 3) at (\Factor*2,-\Factor*3) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 2 4) at (\Factor*2,-\Factor*4) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 2 5) at (\Factor*2,-\Factor*5) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 3 1) at (\Factor*3,-\Factor*1) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 3 2) at (\Factor*3,-\Factor*2) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 3 3) at (\Factor*3,-\Factor*3) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 3 4) at (\Factor*3,-\Factor*4) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 3 5) at (\Factor*3,-\Factor*5) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 4 1) at (\Factor*4,-\Factor*1) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 4 2) at (\Factor*4,-\Factor*2) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 4 3) at (\Factor*4,-\Factor*3) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 4 4) at (\Factor*4,-\Factor*4) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 4 5) at (\Factor*4,-\Factor*5) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 5 1) at (\Factor*5,-\Factor*1) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 5 2) at (\Factor*5,-\Factor*2) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 5 3) at (\Factor*5,-\Factor*3) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 5 4) at (\Factor*5,-\Factor*4) {};
\node[minimum size=0.2cm,
shape=circle,
fill=black!50,
font=\footnotesize] 
(a 5 5) at (\Factor*5,-\Factor*5) {};
\end{tikzpicture}
$$