...
We can also form the symmetry group of $n$ identical objects. There are $n!$ of these symmetries, and we call the corresponding group the \defn{symmetric group} on $n$ objects and denote it $\mathfrak{S}_n$. 
...
\node (4)  [right of=2]   {$\{1,(1\ 3\ 2),(1\ 2\ 3)\}$};
 \node (S3) [above of=1]  {$\mathfrak{S}_3$};
 \draw (Q)   -- (2);
 \draw (Q)   -- (3);
 \draw (Q)   -- (1);
 \draw (2)  -- (S3);
...
\begin{definition}
The \defn{dihedral group} of order $2n$, denoted $\mathfrak{D}_{2n}$, is the group of symmetries of a regular $n$-gon.
\end{definition}
...
\begin{definition}
The \defn{alternating group} on $n$ letters $\mathfrak{A}_n$ is a subgroup of $\mathfrak{S}_n$ of index two corresponding to the even permutations of $\mathfrak{S}_n$.
\end{definition}
...
Lastly, the group $\mathfrak{D}_{12}$ has the following lattice of subgroups. 
\begin{center}
...
\node (d16) [above of  = d122, above of = d122, above of = d122] {$\mathfrak{S}_3$};
\node (d26) [above of  = d222, above of = d222, above of = d222] {$\mathfrak{S}_3$};
\node (d12) [above of  = d24, above of = d24, above of = d24] {$\mathfrak{D}_{12}$};
\draw (d34) -- (d1321);
\draw (1) -- (d112);
\draw (1) -- (d132);
\draw (1) -- (d122);
...
The only group of order $13$ is $\Z/13\Z$, and the only groups of order $14$ are $\Z/14\Z$ and $\mathfrak{D}_{14}$. To classify the groups of order $15$, we will consider groups $G$ of order $pq$ where $p<q$.
...
\item $(\Z/4\Z  \times \Z/2\Z ) \ltimes \Z/2\Z $, the \defn{Pauli group} on one qubit.
\end{enumerate}
...
\end{center}
Then $\Aut(T) = \mathfrak{S}_4 \wr \mathfrak{S}_3$.\fi
\end{enumerate}
\end{example} %mobiles? really?

Wreath products also show up in the Sylow subgroups of symmetric groups. For symmetric groups of index $p^f$ you get an $f$-fold wreath product of $\Z/p\Z$, a sort of tower of wreath products. For instance, if $G=\mathfrak{S}_{27}$, then we get $((\Z/3\Z )\wr (\Z/3\Z)) \wr (\Z/3\Z)$. For others you get products of these towers. See lecture 19 around minute 11 for more detail. % 11:10 (https://www.youtube.com/watch?v=2Ik9m84AT2E&list=PL8yHsr3EFj51pjBvvCPipgAT3SYpIiIsJ&index=19) 
...
%So we have found $3$ ways to ``join'' $\mathfrak{A}_4$ with $\Z/2\Z $.
Let's consider the binary tetrahedral group. Recall that the group $S^3$ of unit quaternions maps to $\SO_3(\R)$. Inside $\SO_3(\R)$ is $\mathfrak{A}_4$ (the rotations of the tetrahedron). We have the following \defn{central extension}
$$
...
$$
\abs{\mathfrak{S}_n} = n!%\frac{e^{-\gamma (n+1)}}{n+1} \prod_1^\infty \left(1 + \frac{n+1}{k}\right)\inv e^{(n+1)/k} % nice
$$ 
...
Coxeter came up with a compact way of writing relations like these: \defn{Coxeter diagrams}. The Coxeter diagram for $\mathfrak{S}_n$ is 
$$
...
$$
These groups $H$ are \defn{extra special groups}. Such a group $H$ has order $p^{1+2n}$, its centre $Z$ has order $p$, and $H/Z \isomto (\Z/p\Z)^{2n}$.
