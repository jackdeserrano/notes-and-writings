...
Suppose that there is a Lie algebra homomorphism \begin{tikzcd}[cramped]
	\mathfrak{g}\ar[r]&\mathfrak{h}
\end{tikzcd} 
where $\mathfrak{g}$ (resp. $\mathfrak{h}$) is the Lie algebra of $G$ (resp. $H$).

If $G$ is simply connected, then this homomorphism induces a Lie group homomorphism \begin{tikzcd}[cramped]
	G\ar[r]& H.
\end{tikzcd}
...
Now, we define a coproduct on $\widehat{R}$. Define \begin{tikzcd}[cramped]
	\Delta : R\ar[r] & R\tensor R
\end{tikzcd}
by $\Delta(x) = x\tensor 1 + 1\tensor x$ and $\Delta(y) = y\tensor 1 + 1\tensor y$.
This induces a homomorphism
\begin{tikzcd}[cramped]
	\widehat{\Delta} : \widehat{R}\ar[r]& \widehat{R}\tensor\widehat{R}.
\end{tikzcd}

We digress to sketch what this coproduct is. 
Take the group algebra $\mathbf{R}[G]$ of a group $G$. This is an associative algebra, so it has a product \begin{tikzcd}[cramped]
	\R[G]\tensor \R[G]\ar[r]&\R[G].
\end{tikzcd}
...
An \defn{enveloping algebra}\index{enveloping algebra} of $\mathfrak{g}$ is a pair $(\varphi,U)$ where $U$ is a unital associative algebra and \begin{tikzcd}[cramped]
	\varphi : \mathfrak{g}\ar[r]& U_-
\end{tikzcd}
is a Lie algebra homomorphism where $U_{-}$ is $U$ with the bracket $[a,b] = ab-ba$.
\end{definition}

\begin{example}[ ]\label{}\text{}
If \begin{tikzcd}[cramped]
	\varphi : \mathfrak{g}\ar[r]& \End(V)
\end{tikzcd}
is a representation of $\mathfrak{g}$ in a vector space $V$, then the pair $(\varphi,\End(V))$ is an enveloping algebra of $\mathfrak{g}$.
\end{example}

\begin{definition}[ ]\label{}\text{}
The \defn{universal enveloping algebra}\index{universal enveloping algebra} of $\mathfrak{g}$ is an enveloping algebra $(\Phi, U(\mathfrak{g}))$ that has the following universal mapping property: for any enveloping algebra $(\varphi, U)$ of $\mathfrak{g}$, there exists a unique associative algebra homomorphism
\begin{tikzcd}[cramped]
	f:U(\mathfrak{g})\ar[r]& U
\end{tikzcd}
...
\begin{tikzcd}[cramped]
	\Phi : \mathfrak{g}\ar[r]& U(\mathfrak{g})_-
\end{tikzcd}
...
It follows that any representation \begin{tikzcd}[cramped]
	\pi : \mathfrak{g}\ar[r]& \End(V)
\end{tikzcd}
extends uniquely to a homomorphism of associative algebras
\begin{tikzcd}[cramped]
	U(\mathfrak{g})\ar[r]& \End (V).
\end{tikzcd}

\begin{theorem}[Poincar\'e--Birkhoff--Witt]\label{pbw}\index{Poincar\'e--Birkhoff--Witt theorem}\text{}
...
One can do this using the coproduct \begin{tikzcd}[cramped]
	\Delta : U(\mathfrak{g})\ar[r]& U (\mathfrak{g})\tensor U (\mathfrak{g}),
\end{tikzcd}
