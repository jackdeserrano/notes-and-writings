...
\end{align*}
where $\ord_p(f)$ is the order of the root of $f$ at $p$. Usually, one supposes that $X$ is compact so that this sum is finite. Such a divisor is called a \defn{principal divisor}\index{principal divisor}. The set of divisors on a compact Riemann surface $X$ is a group denoted $\Div(X)$; the set of principal divisors is a subgroup of $\Div(X)$ denoted $\Princ(X)$. If $d=\sum_{i}^{} n_ip_i$ is a divisor on a compact Riemann surface, then its degree is
\begin{align*}
	\deg d := \sum_{i}^{} n_i.
\end{align*}
