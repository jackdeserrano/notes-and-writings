...
\begin{example}[ ]\label{}
Suppose $G=\mathfrak{S}_n$ acts on $\mathbf{A}^n$ by permuting coordinates. Then, $K[z_1,\hdots, z_n]^G$ is the ring of invariant functions generated by the elementary invariant polynomials. Hence, one sees that $\mathbf{A}^n/\mathfrak{S}_n\isomto \mathbf{A}^n$.\footnote{This works out so nicely because $\mathfrak{S}_n$ is a reflection group.}
\end{example}


If $\Z/2\Z$ acts on the real line by $z\longmapsto -z$, one might think that the quotient is a half-closed interval. This is what one gets by identifying points under the group, but, by the construction above, it's the real line. One identifies $2$ and $-2$, but also identifies $2i$ and $-2i$.\footnote{These are real points of the quotient.}
...
Consider $\mathbf{A}^2$, whose coordinate ring is $K[y_1,y_2]$. Let $G=\Z/n\Z$. Suppose that $G$ is generated by $\sigma$ of order $6$. Let $\zeta$ be an $n$th root of unity. Then, $G$ acts on the coordinate ring by $\sigma y_1 = \zeta y_1$ and $\sigma y_2= \zeta y_2$. One determines the quotient of $\mathbf{A}^2$ by $G$ with the invariants of $K[y_1,y_2]$. Notice that 
\begin{align*}
...
\end{align*}
etc. These six transformations form a group isomorphic to $\mathfrak{S}_3$. We get this affine variety $\mathbf{A}^1 - \{0,1\}$ modulo $\mathfrak{S}_3$. This is an affine variety: If $\lambda\in \mathbf{A}^1-\{0,1\}$, one can think of this set as the curve $\lambda(\lambda-1)\mu = 1\subset\mathbf{A}^2$. What is this space $(\mathbf{A}^1 - \{0,1\}) / \mathfrak{S}_3$, exactly? Its coordinate ring is $K[\lambda, 1/\lambda, 1/(\lambda-1)]$, so one considers the ring of invariants under $\mathfrak{S}_3$. The simplest invariant element is 
\begin{align*}
	j(\lambda) =  \frac{2^8(\lambda^2-\lambda+1)^3}{\lambda^2(\lambda-1)^2}.
\end{align*}
It turns out that $K[\lambda, 1/\lambda, 1/(\lambda-1)]^{\mathfrak{S}_3} \isomto K[j]$. This is the so-called \index{$j$-invariant}\defn{$j$-invariant} of an elliptic curve.
\end{example}
...
Suppose that $G=\Z/n\Z$ is generated by $\sigma$. Suppose further that $\sigma z_1 = \zeta z_1$ and $\sigma z_2 =\zeta^{-1}z_2$ where $\zeta$ is a primitive $n$th root of unity. Invariants under this action are given by $X =z_1^n$, $Y=z_2^n$, and $Z=z_1z_2$. Hence, one has
\begin{align*}
...
\end{align*}
This group is isomorphic to $\mathfrak{S}_3$. Hence, the moduli space of elliptic curves is given by the affine line parameterized by $\lambda$ modulo $\mathfrak{S}_3$. An invariant under this action is the \index{$j$-invariant}$j$-invariant of the elliptic curve $y^2 = x(x-1) (x-\lambda)$:
\begin{align*}
...
\[
\begin{tikzcd}[column sep = 3 em, row sep = 3 em]
	K(\mathfrak{q}_1)\ar[r]&K\\ X_{\mathfrak{q}_0} \ar[u] \ar[r,dashed,"\varphi"] &R\ar[u]
\end{tikzcd}
\]
...
\[
\begin{tikzcd}[row sep = 0 pt]
	\displaystyle \prod_\alpha \mathscr{F} (U_\alpha) \ar[r] &\displaystyle \prod_{\alpha,\nu} \mathscr{F}(V_\nu \times_UU_\alpha)
\end{tikzcd}
\]
because of (+).
...
The $\zeta$ function $\zeta_X$ of a curve $X$ over $\mathbf{F}_{q}$ is closely related to the number of points that $X$ has over some extension $\mathbf{F}_{q^m}$. One denotes this quantity $\normalfont{n}_m$.
...
%		   &\phantom{=} \ + \omega_2t + \frac{\omega_2^2t^2}{2} + \frac{\omega_2^3t^3}{3}+\cdots
	   \sum_{m\ge 1}^{} \frac{t^m}{m} + \sum_{m\ge 1}^{}  \frac{(qt) ^m}{m} + \sum_{m\ge 1}^{} \frac{(\omega_1t)^m}{m} + \cdots + \sum_{m\ge 1}^{} \frac{(\omega_{2g}t)^m}{m}\\
		   &= \sum_{m\ge 1}^{} \normalfont{n}_m\frac{t^m}{m}.
\end{align*}
Therefore,
\begin{align*}
	\left\lvert \normalfont{n}_m -1 - q^m \right\rvert \le 2g(q^{m/2}).
\end{align*}
...
\begin{align*}
	\left\lvert \normalfont{n}_{m}-q^{m}-1 \right\rvert \le 2g(q^{m/2}).
\end{align*}
...
\begin{align*}
	\log(\zeta(s)) := \sum_{m\ge 0}^{}\frac{\normalfont{n}_{m}q^{-ms}}{m}
\end{align*}
where $\normalfont{n}_{m}$ is the number of points on $X$ over $\mathbf{F}_{q^{m}}$.
...
\begin{align*}
	\log(Z(T)) := \sum_{m\ge 0}^{} \frac{\normalfont{n}_{m}T^{m}}{m}.
\end{align*}
Weil found that 
\begin{align*}
...
Define 
\begin{align*}
	\normalfont{n}_m := \# \Fix(\varphi^{m}).
\end{align*}
Then,
\begin{align*}
	\normalfont{n}_m = \sum_{i,j}^{} (-1) ^{i}(\alpha_{ij}) ^{m}
\end{align*}
where the $\alpha_{ij}$s are the eigenvalues of $\varphi$ on $\HH^{i}(X)$.
Thus,
\begin{align*}
	\sum_{m}^{} \frac{\normalfont{n}_m T^{m}}{m} &= \sum_{i,j,m}^{}  \frac{(-1) ^{i}(\alpha_{ij}) ^{m}T^{m}}{m}\\
						 &= \sum_{i,j}^{} (-1) ^{i+1} \log(1-\alpha_{ij}T).
\end{align*}
Now, if
\begin{align*}
	Z(T):= \exp \left( \sum_{m}^{} \frac{\normalfont{n}_m T^{m}}{m} \right) ,
\end{align*}
then
\begin{align*}
...
Take $\varphi = \Frob_{q}$. Then, $\normalfont{n}_m$ is the number of points defined over $\mathbf{F}_{q^{m}}$.
