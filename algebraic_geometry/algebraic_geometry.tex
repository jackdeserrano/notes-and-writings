...
\begin{definition}[ ]\label{}\text{}
Let $X$ be a topological space. 
Let $\mathscr{F}$ and $\mathscr{G}$ be quasicoherent $\mathscr{O}_X$-modules. The \defn{tensor product presheaf}\index{tensor product presheaf} is defined by
\begin{align*}
	(\mathscr{F}\otimes^{\textrm{pre}}_{\mathscr{O}_X} \mathscr{G})(U) :=  \mathscr{F}(U) \tensor_{\mathscr{O}_X(U)}  \mathscr{G}(U)
\end{align*}
...
be the inclusion of an open set $U$ of $X$.
\begin{itemize}
	\item Let $\mathscr{F}$ be an abelian presheaf on $U$. The \defn{extension}\index{extension by zero} of $\mathscr{F}$ by zero is the abelian presheaf on $X$ defined by
		\begin{align*}
			j\sub{p!}(\mathscr{F}) (V) :=
			 \begin{cases}
