...
Grothendieck's \'El\'ements de g\'eom\'etrie alg\'ebrique (EGA) and the Stacks project\footnote{See \url{https://stacks.math.columbia.edu}.} are frequently referenced.
...
\end{align*}
where $P_{i}(T)$ is almost the characterisitc polynomial of $\varphi$ on $\HH^{i}(X)$.
...
\begin{align*}
	...
	\end{cases}
\end{align*}
One wants to show that, if $X$ is a nonsingular, projective, genus-$g$ curve over an algebraically closed field $\Omega$ of characteristic $p$,
\begin{align*}
	...
	\end{cases}
\end{align*}
where $n := n1$ is invertible in $\Omega$. 
...
Recall that the \defn{Brauer group}\index{Brauer group} of a field $K$ consists of (Morita) equivalence classes of central division algebras over $K$. 
...
Similary, sheaves for the \'etale topology assign a set to each \'etale map to $Y$.\footnote{Again, this must be functorial and must satisfy some covering conditions.}
...
\end{align*}
Now, $f'(\alpha)$ is invertible, so one can choose an element $r\in I$ such that $f(\alpha+r)=0$.
Therefore, finite separable extensions are \'etale.
\end{example}
