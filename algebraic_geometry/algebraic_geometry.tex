...
Suppose that $X$ is a scheme. Let $\mathscr{K}=\mathscr{K}_X$ be the sheaf of total quotient rings. Cover $X$ by open subschemes $U_i$. For every subscheme $U_i$, take a section $f_i\in \mathscr{K}\big|_{U_i}$ where $f_i/f_j$ is a unit of $\mathscr{K}\big|_{U_i\cap U_j}$. This gives a Cartier divisor $d$. Now, one wants to associate this Cartier divisor to an invertible subsheaf $\mathscr{L}(d)$ of $\mathscr{K}$ given by sections $g\in \mathscr{K}\big|_{U_i}$ where $gf_i\in \mathscr{O}_{U_i}$ is regular on $U_i$. Again, $f_i$ is an isomorphism from $\mathscr{L}(d)$ to $\mathscr{O}_{U_i}$, so $\mathscr{L}(d)$ is an invertible sheaf. 
...
\begin{align*}
	b_{ij}b_{jk}b_{ki} = 1.
\end{align*}
Thus, $b_{ij}$ is a $1$-\defn{cocycle}\index{cocycle}.

Pick a unit $a_i\in \mathscr{O}(U_i) ^*$. If $b_{ij}=a_ia_j^{-1}$, then $\mathscr{L}$ is trivial. In this case, $b_{ij}$ is a $1$-\defn{coboundary}\index{coboundary}. Let $Z^1$ denote the group of $1$-cocycles and let $B^1$ denote the group of $1$-coboundaries. Then, the group invertible sheaves that are trivial on all $U_i$ is isomorphic to $Z^1/B^1$. All invertible sheaves are given by a direct limit over open coverings given by refinements:
\begin{align*}
	\check H^1 := \varinjlim_{\textrm{open coverings $(U_i)$}} Z^1/B^1.
\end{align*}
