...
\begin{definition}[Artin 1962 Definition 0.1]\label{}\text{}
...
For $\mathscr{F}\in \catn{PAb}(\mathscr{T})$, the sheaf $\mathscr{F}^{\sharp} := \sharp(\mathscr{F})$ is the \defn{associated sheaf}\index{associated sheaf} to $\mathscr{F}$ (the sheafification\index{sheafification} of $\mathscr{F}$). 
...
Let $\mathscr{F}\in \catn{PAb}(\mathscr{T})$. Then, $\mathscr{F}$ \defn{satisfies (+)}\index{satisfying (+)} if and only if, for all 
\[
\begin{tikzcd}[row sep = 0 pt]
