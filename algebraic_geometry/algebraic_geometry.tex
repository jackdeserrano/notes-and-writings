...
\begin{itemize}
	\item A \defn{sheaf}\index{sheaf} of sets on $X$ is a presheaf of sets on $X$ that satisfies the following property: given an open cover $(U_{\alpha}) _{\alpha\in A}$ of an open set $U$ of $X$ and a collection of sections $(s_{\alpha}\in \mathscr{F}(U_{\alpha}))_{\alpha\in A} $ such that, for all $\alpha,\beta\in A$,
		\begin{align*}
...
Let $X$ be a topological space. Let $\mathscr{C}$ be a category with products. A \defn{sheaf}\index{sheaf} on $X$ with values in $\mathscr{C}$ is a presheaf on $X$ with values in $\mathscr{C}$ where, for every open cover $(U_{\alpha}) _{\alpha\in A}$ of an open set $U$ of $X$, the diagram
\[
